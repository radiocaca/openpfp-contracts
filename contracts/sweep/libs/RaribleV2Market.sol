...
library RaribleV2Market {
    address public constant RARIBLE =
    ...
    function _buyAssetForEth(
        ...
        if (success) {
            if (
                raribleBuy.orderLeft.takeAsset.assetType.assetClass ==
                bytes4(keccak256("ETH"))
            ) {
                // In case we got ETH
                (bool _success, ) = msg.sender.call{
                    value: raribleBuy.orderLeft.takeAsset.value
                }("");
                require(
                    _success,
                    "_buyAssetForEth: Rarible market eth transfer failed"
                );
            } else if (
                raribleBuy.orderLeft.takeAsset.assetType.assetClass ==
                ...
                address addr = abi.decode(
                    raribleBuy.orderLeft.takeAsset.assetType.data,
                    (address)
                );
                IERC20(addr).transfer(
                    msg.sender,
                    raribleBuy.orderLeft.takeAsset.value
                );
            } else if (
                raribleBuy.orderLeft.takeAsset.assetType.assetClass ==
